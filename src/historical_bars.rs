...
/// Fetch daily bars from Binance perpetual futures kline API.
/// Returns the last `days` daily bars sorted ascending.
pub async fn fetch_binance_daily_bars(symbol: &str, days: u32) -> Result<Vec<Bar>> {
    let client = reqwest::Client::builder()
        .timeout(std::time::Duration::from_secs(10))
        .build()?;
    let binance_symbol = format!("{}USDT", symbol.to_uppercase());
    let url = format!(
        "https://fapi.binance.com/fapi/v1/klines?symbol={}&interval=1d&limit={}",
        binance_symbol, days
    );
    let resp = client.get(&url).send().await
        .with_context(|| format!("fetching daily klines for {}", binance_symbol))?;
    if !resp.status().is_success() {
        anyhow::bail!("Binance API error {}: {}", resp.status(), resp.text().await.unwrap_or_default());
    }
    let data: Vec<Vec<serde_json::Value>> = resp.json().await
        .context("parsing Binance daily kline response")?;
